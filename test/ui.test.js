...
const pending = (fields = {}) => ({
  ...
  partialText: "途中の返答", status: "checking", error: null, ...fields,
});
  ...
  return {
    ...
    async advance(ms) {
      ...
      for (const [id, timer] of [...timers]) {
        if (timer.at <= time) { timers.delete(id); await timer.callback(); }
      }
      await flush();
    },
  };
...
test("a lost send response preserves text and recovers automatically without another send", async () => {
  let state = { messages: [], pending: null };
  const ui = await boot(async (_url, options) => {
    if (options.method === "POST") {
      state = { messages: [], pending: pending() };
...
test("unknown results stop automatic waiting after 60 seconds and offer check, not retry", async () => {
  const ui = await boot(async () => Response.json({ messages: [], pending: pending() }));
...
test("confirmed failure exposes retry, while a local failed record alone does not", async () => {
  let state = { messages: [], pending: pending({ status: "failed", error: "返答を作れませんでした。入力は保存されています。" }) };
  const ui = await boot(async (url) => {
    if (url === "/api/retry") state = { messages: [], pending: pending({ status: "waiting", partialText: "", error: null }) };
    return Response.json(state);
  });
...
test("an unacknowledged local send is retained and manual resend uses the same ID", async () => {
  const ui = await boot(async (_url, options) => {
    ...
    return Response.json({ messages: [], pending: null });
  });
  await ui.send("こんにちは");
  await ui.advance(60_000);
...
test("another tab's pending message never discards an unaccepted local input", async () => {
  ...
  const ui = await boot(async () => Response.json({ messages: [], pending: pending({ id: "another", text: "別の端末" }) }), saved);
