...
自動確認は60秒で区切り、その後は「もう一度確認」を表示します。生成の失敗を確認できた場合だけ「再試行」で新しい実行を始めます。送信の受け付け自体を確認できない場合は、端末に保存した同じ送信 ID で手動再送できます。サーバーは受け付け済み・完了済みの ID を再実行しません。
...
- `GET /api/messages`: `{ "messages": [...], "pending": null | {...} }` を返す。必要なら実行結果を確認するが、生成は開始しない
...
`pending` は送信内容・途中の文章・状態（`waiting` / `checking` / `failed`）を含みます。未完了の入力がある間、別の入力は受け付けません。
...
テストでは OpenAI API を呼びません。SDK のエラー受信、HTTP 応答の消失、再起動からの復旧、二重実行防止、画面の待機上限と表示文言を検証します。画面のテストは DOM のテスト用実装で動作し、ブラウザ操作はしません。
