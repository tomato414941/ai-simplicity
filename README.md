...
API の契約は操作の受け付けと、session・turn・item・event の意味です。入力が必ず新しい turn を作るとは仮定しません。実行中の入力は同じ turn への追加指示になります。Web の送信中表示・下書き・未確認の送信内容はクライアント側の状態で、API のリソースやイベントには追加しません。

上流エラーの `type`・`param`・`code` と、`Retry-After`・`retry-after-ms`・`x-should-retry`・`x-request-id` は保持します。診断本文に秘密情報が混入するのを避けるため、上流の HTTP エラー `message` は固定文に置き換えます。Cookie などその他の上流ヘッダーは転送しません。
...
Web は `item_id` と `content_index` ごとに表示を更新します。delta ごとの HTTP 確認、履歴全体の描画し直し、サーバーへの履歴書き込みはありません。通常の履歴取得は初期表示・入力の受け付け後・turn 終了時で、通信断後にも取得します。
...
送信の受け付けは HTTP の成功で確認し、未確認の送信内容を端末から取り除きます。受信した本文との一致や新しい turn の発生は待ちません。会話の履歴は API から取得した items だけを表示し、送信直後の未確認入力を履歴の item として合成しません。受け付け後の状態取得に失敗しても、送信が失敗したことにはしません。
...
停止要求中の状態は画面を開いている間だけ保持します。再読み込み後は session と turn を読み直して表示し、停止要求自体の永続化や再送はしません。
...
テストは外部 API を呼びません。独立した二つの公式 SDK クライアントから実際のアプリサーバーに接続し、受け付け後の応答消失・同じキーでの再送・実行中の追加入力・停止と完了の競合・再接続後の履歴共有を検証します。OpenAI 側の応答だけを模擬しており、実サービスへの適合性や複数ユーザーの認可を証明するものではありません。

ほかに HTTP エラーの種別・再試行情報、SSE の分割・文字コード・エラー、ページング、切断復旧、履歴の退避を検証します。Web は DOM のテスト用実装と仮想時計で、実際の配信スクリプトを実行して表示文言・操作を確認します。ブラウザ操作は行いません。
