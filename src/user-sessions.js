import { AgentSession, INSTRUCTIONS, OPTIONS, carry } from "./agent-session.js";
import { grant } from "./tools.js";

export class UserSessions {
  constructor({ client, store, model, tools = [] }) {
    this.client = client;
    this.store = store;
    this.model = model;
    // Sources of what a user's conversation is given beyond the fixed policy (Foundation, when configured).
    this.tools = tools;
  ...
  async create(userId, body) {
    ...
    try {
      ...
      // What this conversation is given: web search always, and whatever its user's sources grant.
      const granted = await grant(this.tools, userId);
      const { tools, instructions } = carry([{ kind: "web_search" }, ...granted.tools]);
      let session;
      try {
        ...
        session = await this.client.beta.agents.sessions.create({
          agent: { model: this.model, instructions: INSTRUCTIONS + instructions, tools, multi_agent: { enabled: false } },
